...
public interface GoodsService {
    ...
    List<GoodsDTO>  selectAll(int pageNo) throws SQLException;
	 ...
	 void insert( GoodsDTO  goodsDTO) throws SQLException;
	   ...
	   * : 수정, 수정완료로 상세보기를 할 때 조회수가 늘어날 수 있기 때문에 판별 필요.
	   *   사용자가 상세보기 누를 때만 작동을 해야 함
	   * */
	  GoodsDTO selectByGoodsCode(int goodsCode, boolean flag)
			throws SQLException;
		 
		 
	  /**
	   * ElectronicsDAOImpl의 모델번호에 해당하는 레코드 삭제 메소드 호출
	   * path를 넣은 이유 : 레코드가 삭제되면 레코드의 첨부된 파일도 없애려고 save파일 경로 찾기
	   * */
	    void delete(int goodsCode, String password, String path) throws SQLException;
